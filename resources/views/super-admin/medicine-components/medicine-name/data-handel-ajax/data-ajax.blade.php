                ...
                return `
                    <tr>
                        <td class="error_data" align="center" text-danger colspan="11">
                            ...
                            Search medicine name not exists on server <span style="color:rgb(220, 53, 69)">!</span>
                        </td>
                    </tr>
                `;
            }
            ...
            return [...rows].map((row, key) => {
                ...
                return `
                    <tr class="table-row user-table-row" id="medic_name" key="${key}">
                        ...
                        <td class="txt_ center" id="medic_name4">
                            <div class="dropdown">
                        ...
                        <td class="tot_complete_" id="medic_name7">
                            <span class="${statusClass}">${statusSignal}</span>
                            <span class="">${statusText}</span>
                        </td>
                        <td class="tot_complete_ center line_height_three" id="medic_name6">
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 404) {
                        ...
                        groups_id.append(`<label class="label_user_edit"> Group-Name : <span class="word_space">${response.messages.medicine_groups ? response.messages.medicine_groups.group_name : ''}</span></label>`);
                ...
                success: function(response) {
                    if (response.status == 400) {
                        $.each(response.errors, function(key, err_value) {
                            ...
                            $('#updateForm_errorList').addClass('alert_show_errors ps-2');
                        ...
                        setTimeout(() => {
                            ...
                            $(".edit_medicine_name").removeClass('is-valid');
                            $(".edit_group_id").closest('.group_nme').find('.select2-container').removeClass('is-select-invalid');
                            $("#save").show('slow');
                            $("#update_btn").attr('hidden', true);
                            $("#update_btn").hide('slow');
                            setTimeout(() => {
