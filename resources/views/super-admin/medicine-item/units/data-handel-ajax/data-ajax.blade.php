<script type="module">
    ...
    $(document).ready(() => {
                ...
                return `
                    <tr>
                        <td class="error_data" align="center" text-danger colspan="11">
                            ...
                            Search unit data not exists on server <span style="color:rgb(220, 53, 69)">!</span>
                        </td>
                    </tr>
                `;
            }
            ...
            return [...rows].map((row, key) => {
                ...
                return `
                    <tr class="table-row user-table-row" id="unit_tab" key="${key}">
                        ...
                        <td class="txt_ center" id="unit_tab3">
                            <div class="dropdown">
                        ...
                        <td class="tot_complete_ center line_height_three" id="unit_tab5">
        ...
        $(document).on('click', '#cancel_btn', () => {
            ...
            $("#units_name").removeClass('is-valid');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 404) {
                        ...
                        const unitsID = $("#unitsID");
                        const unitsName = $("#unitsName");
                        unitsID.append(`<label class="label_user_edit"> Units-ID : <span class="word_space">${response.messages.id}</span></label>`);
                        unitsName.append(`<label class="label_user_edit"> Units-Name : <span class="word_space">${response.messages.units_name}</span></label>`);
                    }
                }
            });
        });
        ...
        $(document).on('click', '#update_btn', function(e){
            ...
            $("#unitsID").addClass('skeleton');
            $("#unitsName").addClass('skeleton');
            ...
            time = setTimeout(() => {
                ...
                $("#unitsID").removeClass('skeleton');
                $("#unitsName").removeClass('skeleton');
                ...
                success: function(response) {
                    if (response.status == 400) {
                        ...
                        setTimeout(() => {
                            ...
                            $('#success_message').text(response.messages);
                            $('.edit_units_name').val("");
                            $(".edit_units_name").removeClass('is-valid');
                            $("#save").show('slow');
                            $("#update_btn").attr('hidden', true);
                            $("#update_btn").hide('slow');
                            setTimeout(() => {
