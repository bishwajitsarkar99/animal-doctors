<script type="module">
    ...
    $(document).ready(() => {
                ...
                return `
                    <tr>
                        <td class="error_data" align="center" text-danger colspan="11">
                            ...
                            Search brand name not exists on server <span style="color:rgb(220, 53, 69)">!</span>
                        </td>
                    </tr>
                `;
            }
            ...
            return [...rows].map((row, key) => {
                ...
                return `
                    <tr class="table-row user-table-row" key="${key}" id="brand_tab">
                        ...
                        <td class="txt_ center" id="brand_tab4">
                            <div class="dropdown">
                        ...
                        <td class="border_ord" id="brand_tab3">${row.medicine_origins ? row.medicine_origins.origin_name : ''}</td>
                        <td class="txt_" id="brand_tab5">
                        ...
                        <td class="tot_complete_ center line_height_three" id="brand_tab6">
        ...
        $(document).on('click', '#cancel_btn', () => {
            ...
            $("#origin_id").closest('.brand_nme').find('.select2-container').removeClass('is-select-invalid');
        ...
        // Select Origin Name Filed Handel
        $(document).on('change', "#origin_id", function(){
            var $selectContainer = $(this).closest('.brand_nme').find('.select2-container');
            var $errorElement = $('.edit_origin_id_error');
        ...
        $(document).on('click', '#save', function(e) {
            ...
            if(originName.trim() == ''){
                $("#origin_id").next('.select2-container').addClass('is-select-invalid');
                $("#origin_id").closest('.brand_nme').append(`<span class="edit_origin_id_error ps-2"> 
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 404) {
                        ...
                        $('.edit_origin_id').val(response.messages.origin_id).trigger('change');

                        const brands_id = $("#brands_id");
                        const origins_id = $("#origins_id");
                        const brands_name = $("#brands_name");
                        brands_id.append(`<label class="label_user_edit"> Brand-ID : <span class="word_space">${response.messages.id}</span></label>`);
                        origins_id.append(`<label class="label_user_edit"> Origin-Name : <span class="word_space">${response.messages.medicine_origins ? response.messages.medicine_origins.origin_name : ''}</span></label>`);
                        brands_name.append(`<label class="label_user_edit"> Brand-Name : <span class="word_space">${response.messages.brand_name}</span></label>`);
                    }
                }
            });
        });
        ...
        $(document).on('click', '#update_btn', function(e){
            ...
            $("#brands_id").addClass('skeleton');
            $("#origins_id").addClass('skeleton');
            $("#brands_name").addClass('skeleton');
            ...
            time = setTimeout(() => {
                ...
                $("#brands_id").removeClass('skeleton');
                $("#origins_id").removeClass('skeleton');
                $("#brands_name").removeClass('skeleton');
                ...
                success: function(response) {
                    if (response.status == 400) {
                        ...
                        setTimeout(() => {
                            ...
                            $('#success_message').text(response.messages);
                            $('.edit_brand_name').val("");
                            $(".edit_origin_id").val(null).trigger('change');
                            $(".edit_brand_name").removeClass('is-valid');
                            $(".edit_origin_id").closest('.group_nme').find('.select2-container').removeClass('is-select-invalid');
                            $("#save").show('slow');
                            $("#update_btn").attr('hidden', true);
                            $("#update_btn").hide('slow');
                            setTimeout(() => {
