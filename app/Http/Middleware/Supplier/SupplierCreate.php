...
class SupplierCreate
{
    ...
    public function handle(Request $request, Closure $next)
    {
        $user = $request->user();
        if (!$user) {
            return abort(416, 'Unauthorized');
        }

        $userEmail = $user->email;

        $roles = SupplierPermission::where('create_status', '!=', 0)
                    ->orderBy('id', 'desc')
                    ->pluck('user_roles_id')
                    ->toArray();

        $accessPermissions = SupplierPermission::where('user_emails_id', $userEmail)
                                ->orWhere('create_status', 1)
                                ->orderBy('id', 'desc')
                                ->get();

        $hasPermission = $accessPermissions->contains(function ($permission) use ($userEmail) {
            return $permission->user_emails_id == $userEmail || $permission->create_status == 1;
        });
